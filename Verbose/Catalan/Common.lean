...
syntax maybeTypedIdent "tal que" ppSpace colGt maybeTypedIdent " and "
...
| `(newStuffCA| $x:maybeTypedIdent tal que $y:maybeTypedIdent and $z) =>
...
| [x, y, z] => do `(newStuffCA| $(← x.stx):maybeTypedIdent tal que $(← y.stx) and $(← z.stx))
| _ => pure default

declare_syntax_cat newFactsCA
syntax colGt namedType : newFactsCA
syntax colGt namedType " and "  colGt namedType : newFactsCA
syntax colGt namedType ", "  colGt namedType " and "  colGt namedType : newFactsCA

def newFactsCAToArray : TSyntax `newFactsCA → Array NamedType
| `(newFactsCA| $x:namedType) => #[toNamedType x]
| `(newFactsCA| $x:namedType and $y:namedType) =>
    #[toNamedType x, toNamedType y]
| `(newFactsCA| $x:namedType, $y:namedType and $z:namedType) =>
...
| `(newFactsCA| $x:namedType and $y) => do
    let xT ← namedTypeToTypeTerm x
    let yT ← namedTypeToTypeTerm y
    `($xT ∧ $yT)
| `(newFactsCA| $x:namedType, $y:namedType and $z) => do
    let xT ← namedTypeToTypeTerm x
    let yT ← namedTypeToTypeTerm y
...
| `(newFactsCA| $x:namedType and $y:namedType) => namedTypeListToRCasesPatt [x, y]
| `(newFactsCA|  $x:namedType, $y:namedType and $z:namedType) => namedTypeListToRCasesPatt [x, y, z]
| _ => default

declare_syntax_cat newObjectCA
syntax maybeTypedIdent "tal que" maybeTypedIdent : newObjectCA
syntax maybeTypedIdent "tal que" maybeTypedIdent colGt " and " maybeTypedIdent : newObjectCA
...
| `(newObjectCA| $x:maybeTypedIdent tal que $new) => do
    ...
    `(∃ $(.mk x'), $newT)
| `(newObjectCA| $x:maybeTypedIdent tal que $new₁ and $new₂) => do
    let x' ← maybeTypedIdentToExplicitBinder x
...
| `(newObjectCA| $x:maybeTypedIdent tal que $new₁ and $new₂) => maybeTypedIdentListToRCasesPatt [x, new₁, new₂]
| _ => default

declare_syntax_cat factsCA
syntax term : factsCA
syntax term " and " term : factsCA
syntax term ", " term " and " term : factsCA

def factsCAToArray : TSyntax `factsCA → Array Term
| `(factsCA| $x:term) => #[x]
| `(factsCA| $x:term and $y:term) => #[x, y]
| `(factsCA| $x:term, $y:term and $z:term) => #[x, y, z]
| _ => #[]

def arrayToFacts : Array Term → CoreM (TSyntax `factsCA)
| #[x] => `(factsCA| $x:term)
| #[x, y] => `(factsCA| $x:term and $y:term)
| #[x, y, z] => `(factsCA| $x:term, $y:term and $z:term)
| _ => default
